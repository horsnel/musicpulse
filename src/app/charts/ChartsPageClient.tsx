...
export function ChartsPageClient({ initialEntries, countryCharts, initialPlatform, initialRegion }: Props) {
  ...
  return (
    <div className="relative z-10">
      ...
      {/* Page header */}
      <div
        className="border-b border-[var(--border)] pt-10 pb-0"
      ...
      >
        <div className="max-w-[1280px] mx-auto px-7">
          <div className="flex items-end justify-between mb-7 gap-6 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span
                  className="inline-flex items-center gap-1.5 text-[11px] font-bold tracking-[0.1em] uppercase rounded-full px-3 py-1"
              ...
              <h1 className="text-[clamp(32px,4vw,48px)] font-black tracking-[-0.04em] leading-[1]">
                Global Charts
              </h1>
              <p className="text-[14px] text-[var(--text3)] font-medium mt-2">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <button
                onClick={cycleRegion}
                className="flex items-center gap-2 bg-[var(--bg2)] border border-[var(--border2)] rounded-[10px] px-3.5 py-[9px] text-[13.5px] font-semibold text-[var(--text)] cursor-pointer transition-all hover:border-[var(--text3)] min-w-[180px]"
              >
                <span className="text-[18px]">{regionMeta.flag}</span>
              ...
              {/* Date */}
              <div className="flex items-center gap-1.5 bg-[var(--bg2)] border border-[var(--border)] rounded-[10px] px-3.5 py-[9px] text-[13px] font-semibold text-[var(--text2)]">
          ...
          {/* Platform tabs */}
          <div className="flex gap-0 overflow-x-auto border-b border-transparent">
            {PLATFORMS.map(p => (
              <button
                key={p.id}
                onClick={() => setPlatform(p.id)}
                className={cn(
                  'flex items-center gap-2 px-5 py-3.5 text-[13px] font-semibold tracking-[-0.01em] border-none cursor-pointer transition-all whitespace-nowrap border-b-2 -mb-px',
                  platform === p.id
      ...
      {/* Main grid */}
      <div className="max-w-[1280px] mx-auto px-7 py-7 grid grid-cols-[1fr_340px] gap-6 items-start">

        {/* Chart table */}
        <div className="mp-card">
          {/* Toolbar */}
          <div className="flex items-center justify-between px-[22px] py-4 border-b border-[var(--border)] gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[14px] font-bold tracking-[-0.02em]">{activePlatform.label}</div>
                <div className="text-[12px] text-[var(--text3)] font-medium">
                  {regionMeta.name} · Apr 27, 2025 · Updated 47 min ago
                </div>
              </div>
            </div>
              ...
              <div className="flex gap-0.5 bg-[var(--bg3)] border border-[var(--border)] rounded-lg p-[3px]">
                {['Top 50', '51–100', '101–200'].map(r => (
                  <button
                    ...
                    className={cn(
                      'px-3 py-[5px] rounded-md text-[11.5px] font-semibold cursor-pointer border-none transition-all',
                      r === posRange
          ...
          {/* Column headers */}
          <div className="grid items-center px-[22px] h-9 border-b border-[var(--border)] bg-[var(--bg3)]"
          ...
          {/* Load more */}
          <div className="flex items-center justify-center gap-2 py-5 text-[13px] font-semibold text-[var(--text3)] cursor-pointer hover:text-[var(--text2)] transition-colors border-t border-[var(--border)]">
        ...
        <div className="flex flex-col gap-5">
          {/* Today's stats */}
          <div className="mp-card">
            <div className="px-[18px] py-[15px] border-b border-[var(--border)] text-[12.5px] font-bold uppercase tracking-[0.06em] text-[var(--text3)]">
              Today's Chart Stats
            </div>
            <div className="grid grid-cols-2 gap-px bg-[var(--border)]">
              {[
                ...
                <div key={s.k} className="bg-[var(--bg2)] p-4">
                  <div className="text-[9px] font-semibold tracking-[0.14em] uppercase text-[var(--text3)] mb-1.5">{s.k}</div>
                  <div className="text-[18px] font-black tracking-[-0.03em]" style={{ color: s.color }}>{s.v}</div>
                  <div className="text-[10.5px] text-[var(--text3)] mt-0.5">{s.sub}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Country charts */}
          <div className="mp-card">
            <div className="flex items-center justify-between px-[18px] py-[15px] border-b border-[var(--border)]">
              <div className="text-[12.5px] font-bold uppercase tracking-[0.06em] text-[var(--text3)]">Country Charts</div>
              <a href="/charts/countries" className="text-[11.5px] font-semibold text-[var(--text3)] no-underline flex items-center gap-1 hover:text-[var(--text2)] transition-colors">
                All 200
            ...
            <div className="p-2">
              {countryCharts.map(c => (
                <a
                  ...
                  className="flex items-center gap-2.5 px-2.5 py-2 rounded-[9px] cursor-pointer transition-colors hover:bg-[var(--bg3)] no-underline"
                >
                  <span className="text-[20px] w-[30px] text-center">{c.flag}</span>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold tracking-[-0.01em] text-[var(--text)]">{c.name}</div>
                    <div className="text-[11.5px] text-[var(--text3)] font-medium mt-0.5 truncate">
                      #1 {c.topSong}
                    </div>
                  </div>
